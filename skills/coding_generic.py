...
def exec_code_edit(
    ...
    **extra,
    ...
    task_text = task or ""
    target_path = path or extra.get("filepath") or ""

    if mode == "full_file" or "save it to" in task_text.lower() or "create" in task_text.lower():
        if not target_path:
            return {
                "ok": False,
                "skill": "coding_generic",
                "error": "Missing path for file creation.",
            }

        content = code or extra.get("content")
...
<body>
  <h1>Placeholder generated file</h1>
  <p>{task_text}</p>
</body>
</html>
"""

        result = write_file(target_path, content)

        return {
            "ok": result.startswith("Successfully"),
            "skill": "coding_generic",
            "backend": "generic_file_writer",
            "speech": result,
            "ui": {
                "type": "code_patch",
                "content": f"--- FILE: {target_path}\n@@\n{content}",
            },
            "data": {
                "path": target_path,
                "mode": mode,
            },
            "error": None if result.startswith("Successfully") else result,
        }

    return {
        "ok": False,
        "skill": "coding_generic",
        "error": "Generic exec_code_edit only supports file creation/full_file mode.",
        "data": {
            "task": task_text,
            "path": target_path,
            "mode": mode,
...
def open_coding_interface(project_dir: str = "") -> dict:
    """Open the full coding workspace UI tab."""
    project_dir = project_dir or CONFIG.get("default_project_dir", "/mnt/e/coding/jarvis-os")

    return {
        "ok": True,
        "speech": {
            "text": "Opening the coding workspace.",
            "priority": "normal",
        },
        "ui": {
            "placement": "tab",
            "format": "coding",
            "title": "CODEX",
            "subtitle": "Claude Code style workspace",
            "summary": f"Coding workspace opened for {project_dir}",
...
TOOLS = [
    ...
    "type": "function",
    "function": {
        "name": "open_coding_interface",
        "description": "Open the full Claude Code / Codex style coding interface in a new UI tab.",
        "parameters": {
            "type": "object",
            "properties": {
                "project_dir": {
                    "type": "string",
                    "description": "Optional project directory to open.",
                }
            },
        },
    },
...
TOOL_MAP = {
    ...
    "open_coding_interface": open_coding_interface,
...
KEYWORDS = {
    ...
    "open_coding_interface": ["open coding interface", "open codex", "open claude code", "coding tab","developer tab","open code workspace",
],
}
