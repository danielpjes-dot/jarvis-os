...
# 0 = no limit, speak all sentences
TTS_SENTENCE_LIMIT=0

# max words before a single long sentence gets chunked
...
touch "$INPUT" "$OUTPUT" "$LAST_INPUT" "$LOG" "$HISTORY" "$CURRENT_SPOKEN"
echo "standby" > "$STATE"
...
final = []
for p in restored:
    if final and len(p.split()) <= 2 and not p.endswith("?"):
        final[-1] += " " + p
    else:
        final.append(p)
...
chunk_long_sentence() {
  local sentence="$1"
  local max_words="${2:-22}"

  printf "%s" "$sentence" | python3 - "$max_words" <<'PY'
import sys, re
...
speak_reply_loop() {
  ...
  while IFS= read -r sentence; do
    ...
    while IFS= read -r sub; do
      ...
      echo "$sub" > "$CURRENT_SPOKEN"
      log "TTS_CHUNK: $sub"

      local pending
      pending=$(cat "$INPUT" 2>/dev/null)
      if [ -n "$pending" ]; then
      ...
      sleep 0.12
    done < <(chunk_long_sentence "$sentence" "$TTS_MAX_WORDS_PER_CHUNK")

  done < <(split_reply_into_sentences "$prepared" "$TTS_SENTENCE_LIMIT")
...
log "Models -> fast=$OLLAMA_FAST reason=$OLLAMA_REASON code=$OLLAMA_CODE deep=$OLLAMA_DEEP"
log "TTS sentence limit = $TTS_SENTENCE_LIMIT (0 means all)"
